class Station {
  constructor(map, booking) {
    ...
    $.get(
      "https://api.jcdecaux.com/vls/v3/stations?contract=Lyon&apiKey=02097fba5ab73ba010c3d0375ab4c4aa19dd9f23"
    )
        ...
        for (let i = 0; i < allStationsData.length; i++) {
          ...
          let colorMarker =
            stationData.status === "OPEN"
              ? stationData.totalStands.availabilities.bikes === 0
                ? "orange"
                : "green"
              : "red";
          let colorStationMarker = L.icon({
            iconUrl: `assets/img/leaf-${colorMarker}.png`
          });
          let markerStation = self.map.generateStationMarker(
            stationData,
            colorStationMarker
          );
          self.map.eventMarkerClick(
            markerStation,
            "click",
  ...
  showInfosStation(stationData) {
    ...
    $("#infos_station_total_station_bikes").text(
      stationData.totalStands.capacity
    );
    if (
    ...
    if (stationData.status === "OPEN") {
      ...
      $("#infos_station_description_status").html(
        "La station est disponible pour la <span>location</span> et le <span>retour</span> de vélos."
      );
      $("#available_bikes")
    ...
    if (stationData.status === "CLOSED") {
      ...
      $("#infos_station_description_status").html(
        "Veuillez nous excuser pour le dérangement occasionné. Merci de vous rapprocher d'une autre station de location."
      );
      $("#available_bikes")
